...
local function default_handler(content,cgi)
	...
	elseif exist then
		...
		mime_code[cgi.fileext] = "application/x-none"
		f_size = lfs.attributes(fname,"size")
	else
		trace_out(err .. "\n")
...
local function php_script(content,env,cgi)
	local ip,port = hub_addr(content)
	local lenv = "SCRIPT_FILENAME=" 
	local filepath = cgi.path
	string.gsub(filepath,"/","\\")
	local script_name = "c:\\gexy\\www.brgguru.com\\" .. filepath .. cgi.filename .. "." .. cgi.fileext
	lenv = lenv .. script_name .. "\n"
	lenv = lenv .. "REQUEST_URI=" .. env.target .. "\n"
	lenv = lenv .. "SERVER_PROTOCOL=HTTP/1.1" .. "\n"
	lenv = lenv .. "QUERY_STRING=" .. cgi.query_string .. "\n"
	lenv = lenv .. "QUERY_METHOD=" .. env.method .. "\n"
	lenv = lenv .. "COMPUTERNAME=GEXIANGYING-PRO\n"
	lenv = lenv .. "REMOTE_ADDR=" .. ip .. "\n"
	if env.method == "POST" and env["content-length"] then
		lenv = lenv .. "CONTENT_LENGTH=" .. env["content-length"] .. "\n"
	end
	local pipe = luaext_pipe.new("c:\\gexy\\php-7\\php-cgi.exe",lenv)
	if env.method == "POST" then
		for i,v in ipairs(Last[content]) do
			pipe:print(v)
		end
	end
	local res = {}
	...
	local first = string.sub(result,1,j)
	local last  = string.sub(result,j+1,-1)
	pipe:closeout()
	pipe:closein()
	local rs = "HTTP/1.1 200 OK\r\n" 
	rs = rs .. "Content-Length: " .. string.len(last) .. "\r\n"
	rs = rs .. first
	hub_send(content,rs)
	hub_send(content,last)
end
...
local function lua_script(content,cgi)
	...
	cgi.json = json
	cgi.response_contenttype= "text/html"		
	cgi.response_status = "200 OK"
	...
	if fh then
		...
		local rs = "HTTP/1.1 " .. cgi.response_status .. "\r\n" 
		rs = rs .. "Content-Type: " .. cgi.response_contenttype .. "\r\n"
		rs = rs .. "Content-Length: " .. string.len(contents) .. "\r\n"
		rs = rs .. "Cache-Control: no-cache\r\n"
		rs = rs .. "\r\n"
		hub_send(content,rs)
...
function process_cmd(content,env,cgi)
	...
	cgi._G = _G
	if cgi.path and cgi.filename then
		if cgi.fileext == "lp" then
			lua_script(content,cgi)
			--elseif cgi.fileext == "php" then
			--	php_script(content,env,cgi)
			--end
		else 
...
function ghub.services.recv(content,str)
	...
	trace_out("process " .. cgi.path .. cgi.filename .. "." .. cgi.fileext .. "  used time " .. 1000 * (t1 - t0) .. " ms\n")
